...
public class LogWriter {
     ...
     public final static LogAPI logger = LogAPI.getInstance(); //Logger.getLogger(SIPMessageStack.class);
    
 	/**
 	 * The subcategory to use when logging
 	 * 
 	 */
 	public static String subCat = "Stack";
...
//	private Logger logger = null;
	...
	public LogWriter() {
		//logger = Logger.getLogger("SIPStack");
		needsLogging = (logger.isDebugEnabled(PC2LogCategory.SIP, subCat) ||
				logger.isTraceEnabled(PC2LogCategory.SIP, subCat));
	}
	
	public int getSequencer() {
		return LogAPI.getSequencer();
	}
...
/*		if (needsLogging) {
		...
		if (logger.isTraceEnabled(PC2LogCategory.SIP, subCat)) {
			logger.trace(PC2LogCategory.SIP, subCat,
					"------------ Traceback ------\n");
			logger.trace(PC2LogCategory.SIP, subCat, "", new Exception());
			logger.trace(PC2LogCategory.SIP, subCat,
					"----------- End Traceback ------\n");
		}
	}
...
/*		if (needsLogging) {
		...
		logger.error(PC2LogCategory.SIP, subCat, ex.getMessage(), ex);
	}
...
/*	private void println(String message) {
	...
	/** Log a debug message into the log file.
	...
	public void logMessage(String message) {
		...
		checkLogFile();
		println(message);*/
		logger.debug(PC2LogCategory.SIP, subCat, message);
	}
	...
	public void logFatal(String message) {
		logger.fatal(PC2LogCategory.SIP, subCat, message);
	}
	/** Log a error message into the log file.
     * @param message message to log into the log file.
     */
	public void logError(String message) {
		logger.error(PC2LogCategory.SIP, subCat, message);
	}
	/** Log a warn message into the log file.
     * @param message message to log into the log file.
     */
	public void logWarn(String message) {
		logger.warn(PC2LogCategory.SIP, subCat, message);
	}
	/** Log a info message into the log file.
     * @param message message to log into the log file.
     */
	public void logInfo(String message) {
		logger.info(PC2LogCategory.SIP, subCat, message);
	}
	/** Log a error message into the log file.
     * @param message message to log into the log file.
     */
	public void logTrace(String message) {
		logger.trace(PC2LogCategory.SIP, subCat, message);
	}
