...
public class TLSMessageProcessor extends MessageProcessor {
	...
	public void start() throws IOException {
		...
		//	PC 2.0	 thread.setName("TLSMessageProcessor");
		...
		thread.setName("SIP - TLS" + sock.getInetAddress().toString() + ":" + sock.getLocalPort());
		thread.start();
	...
	*/
	...
	public void run() {
		...
		while (this.isRunning) {
			try {
				...
				if (tlsMessageChannel != null)
					tlsMessageChannels.put(tlsMessageChannel.getKey(), tlsMessageChannel);
	...
	public synchronized void stop() {
		...
		Collection<TLSMessageChannel> en = tlsMessageChannels.values();
		for ( Iterator<TLSMessageChannel> it = en.iterator(); it.hasNext(); ) {
