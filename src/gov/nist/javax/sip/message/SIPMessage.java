...
public abstract class SIPMessage
	 ...
	 */
	protected LinkedList<SIPHeader> headers;
	...
	// PC 2.0 Add attribute for use with trace tool
	...
	public LinkedList<String> getMessageAsEncodedStrings() {
		...
		synchronized (headers) {
			ListIterator<SIPHeader> li = headers.listIterator();
			while (li.hasNext()) {
	...
	*/
	protected String encodeSIPHeaders()  {
		...
		synchronized (this.headers) {
			ListIterator<SIPHeader> it = this.headers.listIterator();
	...
	*/
	...
	public boolean match(Object other) {
		...
		ListIterator<SIPHeader> li = matchObj.getHeaders();
		while (li.hasNext()) {
			SIPHeader hisHeaders = (SIPHeader) li.next();
			LinkedList<SIPHeader> myHeaders =
			...
			if (hisHeaders instanceof SIPHeaderList) {
				...
				ListIterator<SIPHeader> outerIterator =
	...
	public void replace(
		...
		if (SIPHeader.class.isAssignableFrom(newObject.getClass())) {
			...
			synchronized (this.headers) {
				...
				ListIterator<SIPHeader> li = this.headers.listIterator();
				while (li.hasNext()) {
	...
	public void merge(Object template) {
		...
		for (int i = 0; i < templateHeaders.length; i++) {
			...
			LinkedList<SIPHeader> myHdrs = this.getHeaderList(hdrName);
			if (myHdrs == null) {
	...
	public Object clone() {
		...
		if (this.headers != null) {
			retval.headers = new LinkedList<SIPHeader>();
	...
	public SIPMessage() {
		...
		this.headers = new LinkedList<SIPHeader>();
	 ...
	 */
	public ListIterator<SIPHeader> getHeaders() {
	 ...
	 */
	public void setVia(java.util.List<ViaHeader> viaList) {
		ViaList vList = new ViaList();
		ListIterator<ViaHeader> it = viaList.listIterator();
		while (it.hasNext()) {
			Via via = (Via) it.next();
			vList.add(via);
		}
	...
	@SuppressWarnings("unchecked")
	private LinkedList<SIPHeader> getHeaderList(String headerName) {
		SIPHeader sipHeader =
			(SIPHeader) nameTable.get(headerName.toLowerCase());
		if (sipHeader == null)
			return null;
		else if (sipHeader instanceof SIPHeaderList)
			return (LinkedList<SIPHeader>) (((SIPHeaderList) sipHeader).getHeaderList());
		else {
