...
public class PCHistoryParser extends DefaultHandler{
	 ...
	 */
	private static PCHistory history = null;
	...
	public PCHistoryParser() {
		super();
		logger = LogAPI.getInstance(); 
	}
	
	public PCHistory parse() throws SAXException, IOException {
		xr = XMLReaderFactory.createXMLReader();
		if (xr != null) {
			xr.setContentHandler(this);
			if (PCHistory.HISTORY_FILE.exists()) {
				FileReader reader = new FileReader(PCHistory.HISTORY_FILE);
				xr.parse(new InputSource(reader));
			}
			else {
				history = PCHistory.getInstance();
				writeHistoryFile();
			}
			return history;
		}
	 ...
	 * may need. The PCHistoryParser simply logs that it is starting and 
	 * creates the final History container.
	 */
	@Override
	public void startDocument() throws SAXException {
		logger.info(PC2LogCategory.UI, subCat, "PCHistoryParser - Starting to parse " + PCHistory.HISTORY_FILE);
	 ...
	 * PCHistoryParser begins to separate the various tags into their 
	...
	public void startElement(String uri, String name, String qName, Attributes atts) throws SAXException {
		...
		if (qName.equals(PCSIM2)) {
			...
			case 's':
				...
				break;
			default :
				throw new PC2XMLException(PCHistory.HISTORY_FILE.getName(), 
	 ...
	 * the start and end tags, the PCHistoryParser must be prepared to receive
	...
	static protected void writeHistoryFile() {
		if ((PCHistory.HISTORY_FILE.isFile()  &&
				PCHistory.HISTORY_FILE.canWrite()) ||
				(!PCHistory.HISTORY_FILE.exists())){
			try {
				BufferedWriter out = new BufferedWriter(new FileWriter(PCHistory.HISTORY_FILE, false));
				out.write("<?xml version=\"1.0\"?>\n");
				out.write("<pcsim2 version=\"" + PCHistory.PLATFORM_VERSION + "\">\n");
				...
				for (int i=0; i< PCHistory.MAX_NUM_HISTORY_FILES; i++) {
				...
				if (PCConfigUI.logger != null) {
					PCConfigUI.logger.error(PC2LogCategory.UI, PCConfigUI.subCat, 
