...
public class NistSipMessageHandlerImpl implements ServerRequestInterface,
    ...
    public void processRequest(SIPRequest sipRequest,
            ...
            if (dialog.getRemoteSequenceNumber() >= sipRequest.getCSeq()
                ...
                * Experiment here if you want to re-enable this optimization.
                ...
                dialog.putPending(this, sipRequest.getCSeq()
                        .getSequenceNumber());
               */
            	// PC 2.0 add log message to know when message is being dropped
            	if (LogWriter.needsLogging)
                    sipStackImpl
                            .logMessage("Out of sequence message, dropping message.");
            	return;
        ...
        // PC 2.0 obtain the source IP address and port for the event
        String srcAddr = incomingMessageChannel.getPeerAddress();
        int srcPort = incomingMessageChannel.getPeerPort();
        String transport = incomingMessageChannel.getTransport();
        ...
        if (dialog == null && sipRequest.getMethod().equals(Request.NOTIFY)) {
            ...
            if (ct != null) {
                ...
                if (ct.getDialog().getState() == null) {
                    sipEvent = new RequestEvent(sipProvider, null,
                            (Request) sipRequest, srcAddr, srcPort,
                            sipRequest.getSequencer(), transport);
                } else {
            ...
            if (transaction != null
                ...
                sipEvent = new RequestEvent(sipProvider,
                        ...
                        srcAddr, srcPort, sipRequest.getSequencer(), transport);
            else
                sipEvent = new RequestEvent(sipProvider, null,
                        (Request) sipRequest, srcAddr, srcPort,
                        sipRequest.getSequencer(), transport);
    ...
    public void processResponse(SIPResponse sipResponse,
        ...
        if (this.transactionChannel == null) {
            ...
            ResponseEvent sipEvent = new ResponseEvent(sipProvider, null,
                    (Response) sipResponse, srcAddr, srcPort, 
                    sipResponse.getSequencer(), transport);
            sipProvider.handleEvent(sipEvent, transaction);
            return;
        }
        ...
        ResponseEvent responseEvent = new javax.sip.ResponseEvent(sipProvider,
                (ClientTransaction) transaction, (Response) sipResponse, srcAddr, srcPort,
                sipResponse.getSequencer(), transport);
