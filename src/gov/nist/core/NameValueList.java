...
public class NameValueList extends GenericObjectList {
	...
	*/
	...
	public void set(NameValue nv) {
		// PC 2.0 Add if test for deleting an existing parameter
		// if it returns true, logger error message.
		if (this.delete(nv.name)) {
			logger.logError("Duplicate parameter[" + nv.name + "] detected in header. (RFC347)");
		}
		this.add(nv);
	}
