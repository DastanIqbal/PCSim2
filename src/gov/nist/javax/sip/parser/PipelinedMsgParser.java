...
public final class PipelinedMsgParser implements Runnable {
	...
	// PC 2.0 change code to accommodate STUN keepalives for TCP and TLS.
	// Basically if we receive a buffer of only "\r\n\r\n" we need to
	// respond with pong ("\r\n");
	private boolean CRLF1 = false;
	private boolean CRLF2 = false;
		...
		try {
			while (true) {
				...
				while (true) {
					try {
						...
						if (line1.equals("\n")) {
							...
							if (CRLF2) {
								if (sipMessageListener instanceof TCPMessageChannel) {
									((TCPMessageChannel)sipMessageListener).sendKeepAlive();
								}
								else if (sipMessageListener instanceof TLSMessageChannel) {
									((TLSMessageChannel)sipMessageListener).sendKeepAlive();
								}
								CRLF2 = false;
								firstLine = true;
							}
				...
				if (sipMessageListener != null) {
					try {
						sipMessageListener.processMessage(sipMessage, nbytes);
					} catch (Exception ex) {
