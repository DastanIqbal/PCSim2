...
public class StringMsgParser {
	...
	private Vector<String> messageHeaders; // Message headers
	...
	public StringMsgParser() {
		super();
		messageHeaders = new Vector<String>(10, 10);
		bufferPointer = 0;
		currentLine = 0;
		readBody = true;
	}
...
//		int sip_message_size = 0; // # of lines in the sip message
		...
		messageHeaders = new Vector<String>(); // A list of headers for error reporting
		try {
			while (tokenizer.hasMoreElements()) {
				String nexttok = tokenizer.nextToken();
				if (nexttok.equals("\n")) {
...
//				sip_message_size++;
		...
		if (!firstLine.startsWith(SIPConstants.SIP_VERSION_STRING)) {
			...
			try {
				...
				if (this.parseExceptionListener != null)
					...
					throw ex;
				// PC 2.0 Add a call to the inspection listener to notify it that 
				// a failure occurred in stack while trying to parse a message
				InspectionListener.getInstance().inspectionFailure();
		...
		for (int i = 1; i < messageHeaders.size(); i++) {
			...
			try {
				...
				// PC 2.0 add if test for listener to remove termination of processing
				if (this.parseExceptionListener != null) {
					this.parseExceptionListener.handleException(
					ex,
					sipmsg,
