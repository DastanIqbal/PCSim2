...
public abstract class SIPMessageStack {
	...
	*/
	...
	// PC 2.0 Add a new property to control whether 
	// STUN processing is allowed over the message
	// processor or not
	protected boolean embeddedStun = false;
	
	protected boolean compressStunMsg = false;
	...
	*/
	...
	// PC 2.0 Add a flag specifying whether a TCP/TLS
	// socket can be reestablished during a test or not.
	protected boolean reestablishTCP = false;
	...
	public void logBadMessage(String message) {
		...
		LogWriter.logger.warn(PC2LogCategory.SIP, "Stack", "Dropping bad message:\n" + message);
	}
	...
	*/
	...
	public SIPMessageStack() {
		...
		// PC 2.0 There are delivery issues when the thread pool is infinite. 
		// Messages may be delivered to the application layer out of sequence.
	...
	public void stopStack() {
		...
		synchronized (this.messageProcessors) {
			...
			for (int processorIndex = 0;
				...
				processorIndex++) {
				// PC 2.0 Add stop to all of the processors
				processorList[processorIndex].stop();
