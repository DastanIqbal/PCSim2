...
// PC 2.0 added new import for sendMessage method
...
public final class SipProviderImpl implements javax.sip.SipProvider,
    ...
    public void addSipListener(SipListener sipListener)
        ...
        synchronized (sipStack) {
            Iterator<SipProvider> it = sipStack.getSipProviders();
            while (it.hasNext()) {
    ...
    public ClientTransaction getNewClientTransaction(Request request)
        ...
        if (sipRequest.getTopmostVia() != null) {
            ...
            Iterator<ListeningPoint> it = sipStack.getListeningPoints();
            boolean found = false;
        ...
        Iterator<HopImpl> it = sipStack.getRouter().getNextHops(request);
    ...
    /**
     * @since PC 2.0
     * Sends specified {String}and returns void i.e.
     * no transaction record is associated with this action. This method implies
     * that the application is functioning statelessly specific to this message,
     * hence the underlying SipProvider acts statelessly.
     * <p>
     * Once the message has been passed to this method, the SipProvider
     * will forget about this message. No transaction semantics will be
     * associated with the message and no retranmissions will occur on the
     * message by the SipProvider, if these semantics are required it is the
     * responsibility of the application not the JAIN SIP Stack.
     * <ul>
     * <li>Stateless Proxy - A stateless proxy simply forwards every request it
     * receives downstream and discards information about the message
     * once the message has been forwarded. A stateless proxy does not have any
     * notion of a transaction.
     * </ul>
     * 
     * @param msg
     *            The message to send statelessly
     * @param seq
     * 			The sequence number of the message relative to the start of the test
     * @param host 
     * 			The host to send the message to
     * @param port
     * 			The port to send the message to
     * @param transport
     * 			The transport protocol to be used to send the message.
     * @throws SipException
     *             if implementation cannot send message for any reason
     */
    public void sendMessage(String msg, int seq,
    		String host, int port, String transport) throws SipException {
    	...
    	try {
    		...
    		if (messageChannel != null) {
    			...
    			messageChannel.sendMessage(msg.getBytes(), seq, addr, port, false);
    			return;
    		} 
    	...
    	// Could not find a suitable hop to forward the request.
    	throw new SipException("could not send message(" + msg + ") to " + host + ":" + 
    			port + " using " + transport);
