...
//	private int port;
	 ...
	 */
	protected LinkedList<DatagramPacket> messageQueue;
	...
	*/
	...
	// PC 2.0 add a reference for the StunListener so individual messages
	// can be forwarded to the STUN stack for processing.
	protected static EmbeddedStunListener stunListener = EmbeddedStunListener.getInstance();
	
	// PC 2.0 - This allows the STUN stack to share the socket that the processor is
	// sending/receiving packets. The STUN stack will automatically transmit any
	// response on the same socket that it was received.
	protected StunMessageProcessor smp = null;
	...
	protected UDPMessageProcessor(SIPMessageStack sipStack, int port)  
		...
		this.messageQueue = new LinkedList<DatagramPacket>();
//		this.port = port;
...
// PC 2.0 Block out old obsolete STUN code.
/*		if (sipStack.stunServerAddress != null) {
		...
		if (sipStack.embeddedStun) {
			smp = stunListener.createStunAccessPoint(sock);
		}
		...
		thread.setName("SIP - UDP" + sock.getLocalAddress().toString() + ":" + sock.getLocalPort());
		thread.start();
	}
	...
	public void run() {
		...
		while (this.isRunning) {
			...
			try {
				...
				// PC 2.0 Add the ability for each message processor to deliver a
				// Stun packet it receives to the STUN stack for processing.
				if (stunListener.isStunPacket(packet)) {
					if (smp != null) {
						smp.addEmbeddedPacket(packet.getData(),
			    				 packet.getLength(), packet.getAddress(), packet.getPort(),
			    				 sock.getLocalAddress().getHostAddress(), getPort());
					}
					else {
						if (LogWriter.needsLogging)
							getSIPStack().logWriter.logMessage(
								"UDPMessageProcessor: Dropping STUN message because Embedded STUN setting was false.");
					}
				}
				// This is not a STUN packet, so continue with normal processing
				else {
