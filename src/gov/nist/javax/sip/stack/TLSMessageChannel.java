...
// PC 2.0 - Add the PONG message for responding to STUN KeepAlive messages 
// on the SIP signaling channel.
    ...
    throws IOException {
        ...
        // PC 2.0 - make sure the peer protocol gets set
    ...
    // PC 2.0 make this method public for STUN KeepAlive
    // processing.
    ...
    // PC 2.0 add a method to simply send the PONG response for the 
    // STUN Keep Alive on this channel.
    public void sendKeepAlive() {
    	 try {
    		 // Make sure it has been cached before sending answer.
    		 if (! this.isCached) {
                 ( (TLSMessageProcessor)
    		 ...
    		 this.stack.logWriter.logInfo(">>>>> RX:\tLength = " + (PONG.getBytes().length * 2)
    				+ "\nReceived on IP|Port=" + myAddress + "|" + myPort  
    		    	+ "\nFrom IP|Port=" + peerAddress + "|" + peerPort
    		   		+ "\nSequencer=" + this.stack.logWriter.getSequencer()
             		+ "\nTransport=" + peerProtocol
             		+ (stack.compressStunMsg ? ("\nSTUN TLS KeepAlive") 
             				:  ("\nSTUN TLS KeepAlive" + "\n[" + PONG + PONG + "]")));
    		 if (stack.embeddedStun) {
    			 sendMessage(PONG.getBytes(), false);
    			 this.stack.logWriter.logInfo("<<<<< TX:\tLength = " + PONG.getBytes().length 
    			 ...
    			 if (LogWriter.needsLogging)
						getSIPStack().logWriter.logMessage(
							"TLSMessageProcessor: Dropping STUN PING message because Embedded STUN setting was false.");
    		 }
    	 }
    	 catch (IOException ioe) {
    		 this.stack.logWriter.logError("TCPMessageChannel could not send STUN KeepAlive message to peerAddr=[" 
    				 + peerAddress + "] peerPort=[" + peerPort + "].\n Exception:" + ioe.getMessage() + "\n" + 
    				 ioe.getStackTrace());
    	 }
    ...
    private void sendMessage(byte[] msg, boolean retry) throws IOException {
        ...
        if (sock != mySock && sock != null) {
            try {
            	// PC 2.0 necessary to determine whether the STUN's NetAccessPoint
                // needs to be updated or not.
            	if (mySock != null && stack.reestablishTCP) {
                    mySock.close();
            	}
            } catch (IOException ex) {
            }
...
//      PC 2.0 change if statement for new log processing
    ...
    throws IOException {
        ...
        // sent fom the socket
		this.stack.logWriter.logInfo("<<<<< TX:\tLength = " + message.length 
            		+ "\nSent from IP|Port=" + myAddress + "|" + myPort  
            		+ "\nTo IP|Port=" + peerAddress + "|" + peerPort
            		+ "\nSequencer=" + seq
            		+ "\nTransport=" + peerProtocol
	 ...
	 */
/*	public void sendMessage(String message, InetAddress receiverAddress, 
			int receiverPort, String transport ) throws IOException {
		long time = System.currentTimeMillis();
    ...
    throws ParseException {
       // PC 2.0 Log all exceptions from the parser 
       // if (LogWriter.needsLogging)
     ...
     */
    public void processMessage(SIPMessage sipMessage, int nbytes)  throws Exception {
        try {
        	// PC 2.0 add logging statement for all SIP messages 
			// received on the socket
        	int seq = stack.logWriter.getSequencer();
			sipMessage.setSequencer(seq);
			this.stack.logWriter.logInfo(">>>>> RX:\tLength=" + nbytes
    		+ "\nReceived on IP|Port=" + myAddress + "|" + myPort  
    		+ "\nFrom IP|Port=" + peerAddress + "|" + peerPort
    		+ "\nSequencer=" + seq
    		+ "\nTransport=" + peerProtocol
...
//                  PC 2.0 change if statement for new log processing
    ...
    //    String message;
        ...
        this.isRunning = true;
        // PC 2.0 Add the ability for each message processor to deliver a
		// Stun packet it receives to the STUN stack for processing.
//		if (stack.embeddedStun) {
//			smp = stunListener.createStunAccessPoint(mySock);
//		}
        try {
            while (true) {
                try {
                	...
                	// because this is the same message as the STUN keep alive for TCP and TLS.
//                    try {
//                        hispipe.write("\r\n\r\n".getBytes("UTF-8"));
//                    } catch (Exception e) {
