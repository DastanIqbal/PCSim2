...
public abstract class Parser extends ParserCore implements TokenTypes {
	...
	protected String sipVersion() throws ParseException {
		...
		try {
			...
			if (!tok.getTokenValue().equals("SIP"))
				// PC 2.0 change the syntax and have the created exception thrown
				// to the calling class
				throw createParseException("Expecting SIP. (RFC337)");
			lexer.match('/');
			tok = lexer.match(ID);
			if (!tok.getTokenValue().equals("2.0"))
				// PC 2.0 change the syntax and have the created exception thrown
				// to the calling class
				throw createParseException("Expecting SIP/2.0. (RFC336)");

			return "SIP/2.0";
		} 
		// PC 2.0 add catch to log error 
	...
	protected String method() throws ParseException {
		try {
			if (debug)
				dbg_enter("method");
			Vector<Token> tokens = this.lexer.peekNextToken(1);
